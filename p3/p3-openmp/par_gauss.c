 ...
 * ========== ANALYSIS ==========
 *
 * https://docs.google.com/document/d/1fNGkEzBM9LczoBvx5OFHYRbPQYsBr-LYMnCmDMfctk8/edit?usp=sharing 
 *
 */
...
#ifdef _OPENMP
#   pragma omp parallel for default(none) shared(A,n,triangular_mode,seed) private(row,col)
#endif
    for (row = 0; row < n; row++) {
        col = triangular_mode ? row : 0;
        for (; col < n; col++) {
            if (row != col) {
    ...
    double tmp;
#ifdef _OPENMP
#   pragma omp parallel for default(none) shared(A,b,n) private(row,col,tmp)
#endif
    for (row = 0; row < n; row++) {
        tmp = 0.0;
        for (col = 0; col < n; col++) {
...
#ifdef _OPENMP
#   pragma omp parallel for default(none) shared(A,b,n,pivot) private(row, col, coeff)
#endif
...
void back_substitution_row()
{
    ...
    for (row = n-1; row >= 0; row--) {
        tmp = b[row];
#ifdef _OPENMP
#       pragma omp parallel for default(none) reduction(+:tmp) shared(A,x,n,row) private(col)
#endif
...
void back_substitution_column()
{
    ...
    for (col = n-1; col >= 0; col--) {
        x[col] /= A[col*n + col];
#ifdef _OPENMP
#       pragma omp parallel for default(none) shared(A,x,n,col) private(row) // works with 4000+ size
#endif
